...
public interface AssessStudentConductDetailRepository extends JpaRepository<AssessStudentConductDetail, Integer> {
    ...
    @Query(nativeQuery = true, value = "select b.*, sum(case a.check_date when 'K' then 1 else 0 end) as number_off, sum(case a.check_date when 'P' then 1 else 0 end) as number_off_allowed from (SELECT asd.* FROM attendance_student as ats \n" +
            ...
            "\t\t\t\tjoin gradebook gb on gb.school_year = asco.school_year and asco.semester = gb.semester and asco.class_code = gb.class_code\n" +
            "\t\t\t\tjoin gradebook_subjects_details gsd on gsd.parent_code = gb.code\n" +
            "\t\t\t\twhere (asco.semester = 1 or asco.semester = 2) and  asco.class_code like :classCode and asco.school_year like :year and gsd.student_code like s.code) as evaluate ,\n" +
