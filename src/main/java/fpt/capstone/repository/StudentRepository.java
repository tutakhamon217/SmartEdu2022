...
public interface StudentRepository extends JpaRepository<Student, Integer> {
    ...
    @Query(value="SELECT new fpt.capstone.vo.StudentVo(s.id, s.code, s.fullName, s.sex, s.birthDay, c.name, d.name, s.status) from Student s JOIN StudentHistory h on s.code = h.studentCode and h.year like :years left join ClassRoom c on h.currentClassCode = c.code left join Departments d on s.deptId = d.id WHERE (s.fullName LIKE %:search_word% or s.code LIKE %:search_word%) and (:class_code like '' or h.currentClassCode like :class_code)  and (:grade_level like '' or CAST(c.gradeLevel AS string) like :grade_level) and ( :status like '' or CAST(s.status AS string) like :status) ")
    List<StudentVo> getAllStudentByNameOrCodeInClassAllStatus(@Param("status") String status, @Param("class_code") String class_code, @Param("search_word") String search_word,  @Param("grade_level") String grade_level, @Param("years") String years);
