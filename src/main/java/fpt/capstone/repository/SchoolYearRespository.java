...
public interface SchoolYearRespository extends JpaRepository<SchoolYear, Integer> {
    ...
    @Query(nativeQuery = true, value = "select \n" +
            "case when (sy.years in (select distinct years from school_year s where s.years in (select distinct school_year from schedule_timetable)) or SUBSTRING(sy.years,6,10)<=year(current_date)+1 ) \n" +
