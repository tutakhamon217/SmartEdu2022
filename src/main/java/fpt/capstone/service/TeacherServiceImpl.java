...
public class TeacherServiceImpl implements TeacherService {
    ...
    public ServiceResult<Map<String, Object>> getTeacherDetailById(Integer teacherId) {
        ServiceResult<Map<String, Object>> serviceResult = new ServiceResult<>();
        Map<String, Object> teacher = repository.getTeacherDetailById(teacherId);
        if (teacher.size() == 0) {
    ...
    public ServiceResult<Boolean> insert(TeacherForm form) {
        ...
        if (form.getContractType() == null) {
            return new ServiceResult<>(HttpStatus.BAD_REQUEST, "Thiếu thông tin hợp đồng", true);
        }
        if (form.getContractType() == null) {
            return new ServiceResult<>(HttpStatus.BAD_REQUEST, "Thiếu thông tin ngày ký hợp đồng", true);
        }
        if (form.getPhone() == null) {
            ...
            if (!check) {
                return new ServiceResult<>(HttpStatus.FAILED_DEPENDENCY, "Số điện thoại sai định dạng", true);
            }
        }
        ...
        if (form.getIdentityCard().length() == 0) {
            if (!form.getIdentityCard().matches(Regex.onlyNumber)){
                return new ServiceResult<>(HttpStatus.BAD_REQUEST, "ố CMND/CCCD sai định dạng", true);
            }
            if (form.getIdentityCard().length() != 12||form.getIdentityCard().length() != 9) {
                return new ServiceResult<>(HttpStatus.BAD_REQUEST, "Số CMND/CCCD chỉ có 9 hoặc 12 số ", true);
            }
        } else {
            if (form.getIdentityCard().length() != 12&&form.getIdentityCard().length() != 9) {
        ...
        Teacher teacher = new Teacher(form.getImage(), LocalDateTime.now(), username, form.getFullName(), form.getCode(), form.getDeptId(), LocalDateTime.parse(form.getStartDate(), ft), form.getContractType(), form.getPhone(),
                form.getEmail(), LocalDate.parse(form.getBirthDay(), ft2), form.getHomeTown(), form.getNation(), form.getPermanentAddress(), form.getSocialInsuranceNumber(), form.getIdentityCard(),
                form.getIssuedAddress(), LocalDateTime.parse(form.getIssuedDate(), ft), form.getMarriageStatus(), form.getSex(), 0, form.getReligion(), form.getTemporaryAddress());

        repository.save(teacher);
        boolean insertUser = userService.insertUserForTeacher(form.getCode(), form.getFullName(), form.getPhone(), form.getImage(), form.getEmail(), form.getAuthorities());
        if (!insertUser) {
            return new ServiceResult<>(HttpStatus.BAD_REQUEST, "Tạo tài cán bộ không thành công", true);
        }
    ...
    public ServiceResult<Boolean> update(TeacherForm form, Integer id) {
        ...
        if (form.getIdentityCard().length() == 0) {
            return new ServiceResult<>(HttpStatus.BAD_REQUEST, "Thiếu thông tin CMND/CCCD", true);
        } else {
            if (!form.getIdentityCard().matches(Regex.onlyNumber)){
                return new ServiceResult<>(HttpStatus.BAD_REQUEST, "Số CMND/CCCD sai định dạng", true);
            }
        ...
        teacherOld.setUpdateName(username);
        ...
        boolean updateUser = userService.updateUserForTeacher(form.getCode(), form.getFullName(), form.getPhone(), form.getImage(), form.getEmail(), form.getAuthorities());
        if (!updateUser) {
