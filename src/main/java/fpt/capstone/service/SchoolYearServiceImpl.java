...
public class SchoolYearServiceImpl implements SchoolYearService {
    ...
    public ServiceResult<Map<String, Object>> saveSchoolYear(SchoolYearForm schoolYearForm) {
        ...
        ServiceResult<Map<String, Object>> serviceResult = new ServiceResult<>();
        Map<String, Object> output = new HashMap<>();
        if (LocalDateTime.parse(schoolYearForm.getToDate1(), ft).isBefore(LocalDateTime.parse(schoolYearForm.getFromDate1(), ft))) {
        ...
        if (schoolYearRepository.getSchoolYearByYearsOrderBySemesterAsc(yearPast2 + "-" + yearPast1).size() > 0) {
            if (LocalDateTime.parse(schoolYearForm.getFromDate1(), ft).isBefore(schoolYearRepository.getSchoolYearByYearsOrderBySemesterAsc(yearPast2 + "-" + yearPast1).get(schoolYearRepository.getSchoolYearByYearsOrderBySemesterAsc(yearPast2 + "-" + yearPast1).size() - 1).getToDate())) {
        ...
        if (schoolYearForm.getSemesterAmount().equals("2")) {
            ...
            if (LocalDateTime.parse(schoolYearForm.getToDate2(), ft).isBefore(LocalDateTime.parse(schoolYearForm.getFromDate2(), ft))) {
            ...
            if (schoolYearRepository.getSchoolYearByYearsOrderBySemesterAsc(yearCurrent + "-" + yearNext).size() > 0) {
                if (LocalDateTime.parse(schoolYearForm.getToDate2(), ft).isAfter(schoolYearRepository.getSchoolYearByYearsOrderBySemesterAsc(yearCurrent + "-" + yearNext).get(0).getFromDate())) {
                    serviceResult.setMessage("Ngày kết thúc học kỳ cuối của năm học thêm mới không được vượt quá ngày đàu tiên của năm tiếp theo ");
                    serviceResult.setStatus(HttpStatus.BAD_REQUEST);
                    return serviceResult;
                }
            }
            ...
            if (schoolYearRepository.getSchoolYearByYearsOrderBySemesterAsc(yearCurrent + "-" + yearNext).size() > 0) {
                if (LocalDateTime.parse(schoolYearForm.getToDate1(), ft).isAfter(schoolYearRepository.getSchoolYearByYearsOrderBySemesterAsc(yearCurrent + "-" + yearNext).get(0).getFromDate())) {
                    serviceResult.setMessage("Ngày kết thúc học kỳ cuối của năm học thêm mới không được vượt quá ngày đàu tiên của 1 năm tiếp theo ");
                    serviceResult.setStatus(HttpStatus.BAD_REQUEST);
                    return serviceResult;
                }
            }
        ...
        try {
            ...
            e.printStackTrace();
            serviceResult.setMessage("fail:" + e.getMessage());
            serviceResult.setStatus(HttpStatus.BAD_REQUEST);
        }
        return serviceResult;
    ...
    public ServiceResult<Boolean> checkStartEndSemester(String years, Integer semester) {
        ...
        if ((startDate.isBefore(ltn) || startDate.isEqual(ltn)) && (endDate.isAfter(ltn) || endDate.isEqual(ltn))) {
            ...
            return new ServiceResult<>(HttpStatus.BAD_REQUEST, "Thời gian đánh giá bắt đầu " + dtf.format(startDate) + " và đã kết thúc vào " + dtf.format(endDate), null);
        }
    }
