...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public boolean insertUserForTeacher(String login, String fullName, String phone, String ImageUrl, String email, Set authorities) {
    ...
    @Override
    public boolean updateUserForTeacher(String login, String fullName, String phone, String ImageUrl, String email, Set authorities) {
        Set<Authority> roles = convert.convertStringToAuthority(authorities);
        try {
    ...
    @Override
    public User insertUserForStudent(String login, String fullName, String phone, String ImageUrl, String email, Set authorities) {
        Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        String username;
        User userSaved = null;
        if (principal instanceof UserDetails) {
    ...
    public ServiceResult<Boolean> resetPassword(String login) {
        ...
        if (user == null) {
            ...
            String phone = userRepository.getPhoneNumberByLogin(login);
            ...
            try {
                ...
                LocalDateTime resetDate = LocalDateTime.now().plusMinutes(1);
                user.setResetDate(resetDate);
                user.setResetKey(OTP);
                userRepository.save(user);
                return new ServiceResult<>(HttpStatus.OK, "Mã OTP đã được gửi đến số điện thoại " + phone, true);
            } catch (IOException e) {
                e.printStackTrace();
    ...
    public ServiceResult<Boolean> checkOTPandResetPass(String otp, String login) {
        try {
            ...
            if (user == null) {
                ...
                if (LocalDateTime.now().isAfter(user.getResetDate().now())) {
            ...
            return new ServiceResult<>(HttpStatus.OK, "Cập nhật mật khẩu thất bại", true);
        }
    }
