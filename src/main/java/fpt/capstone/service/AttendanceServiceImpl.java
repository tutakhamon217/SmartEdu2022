...
public class AttendanceServiceImpl implements AttendanceService{
    ...
    public ServiceResult<Boolean> updateAttendace(UpdateAttendanceDTO form) {
        form.getAttendances().stream().forEach((a) -> {
            ...
            Optional<AttendanceStudentDetail> detail = detailRepository.getAttendanceByDate(a.getCode(), LocalDate.parse(a.getDate()));
            if(detail.isPresent())
            {
            ...
            Student student = studentRepository.getByCode(a.getCode()).orElseThrow(() -> new ValidationException("code", "Code không tồn tại"));
            ...
            String semester = schoolYearRespository.getSemester(attendanceDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime()).orElseThrow(() -> new ValidationException("semester", "Kì học không tồn tại"));
            Optional<String> parentCodeOptional = attendanceStudentRepository.getCode(form.getYear(), semester, Integer.toString(month));
            ...
            temp.add(new AttendanceStudentDetail(100, a.getCode(), student.getFullName(), a.getCheckDate(), parentCode, attendanceDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate()));
