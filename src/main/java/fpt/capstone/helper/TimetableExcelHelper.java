...
public class TimetableExcelHelper {
    ...
    public static ByteArrayInputStream export(String years, String semester, String className, String applyDate,
        ...
        try {
            ...
            throw new ValidationException("file", "Không thể xuất file, nguyên nhân: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            for (int row = 2; row <= timetableSheet.getLastRowNum(); ++row) {
                ...
                if (rowValue == null) {
                    ...
                    // 1));
                    errors.add(String.format("(%d, ) Dòng rỗng", row + 1));
                    continue;
                }
                // System.out.println(rowValue.getLastCellNum());
                int lastCol = rowValue.getLastCellNum();
                if (lastCol < 3) {
                    errors.add(String.format("(%d, ) Thiếu dữ liệu", row + 1));
                    continue;
                }
                if (lastCol > 13) {
                    errors.add(String.format("(%d, ) Thừa dữ liệu", row + 1));
                    continue;
                }
                ...
                try {
                    ...
                    errors.add(String.format("(%d, %d) Sai số thứ tự", row + 1, 1));
                    continue;
                }

                if (Double.compare(no, row - 1) != 0) {
                    errors.add(String.format("(%d, %d) Sai số thứ tự, ở bản tính: %s, giá trị đúng: %.1f", row + 1, 1,
                ...
                if (!classroomRespository.isExist(gradeLevel, years, classCode)) {
                    errors.add(String.format("(%d, %d) Lớp học %s không hợp lệ", row + 1, 2, rowValue.getCell(1)));
                    continue;
                }
                ...
                if (!Stream.of(DAYS_OF_WEEK).anyMatch(x -> dayOfWeek.trim().equals(x))) {
                    errors.add(String.format("(%d, %d) %s không phải là thứ trong tuần hợp lệ", row + 1, 3,
                            rowValue.getCell(2)));
                    continue;
                }
                ...
                for (int i = 3; i <= 12; ++i) {
                    ...
                    if (subjectCode != null
                        ...
                        errors.add(
                                String.format("(%d, %d) Môn học %s không hợp lệ", row + 1, i + 1, rowValue.getCell(i)));
                        continue;
                    }
            ...
            throw new ValidationException("file", "Unreadable file");
        }
