...
public class SubjectClassController {
    ...
    @GetMapping(value = "/subjectClass/load", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<ServiceResult<Map<String, Object>>> loadScreen() throws ParseException {
        ServiceResult<Map<String, Object>> subjects = subjectClassService.loadScreen();
        return ResponseEntity.ok(subjects);
    }
    ...
    public ResponseEntity<ServiceResult<Map<String, Object>>> getSubjectClassClassIdSubjectIdSemester1(@RequestParam(required = false) Integer classId, @RequestParam(required = false) Integer semester) throws ParseException {
        ServiceResult<Map<String, Object>> results = subjectClassService.getSubjectClassByClassIdSubjetIdSemester(classId, semester);
        return ResponseEntity.ok(results);
    }
